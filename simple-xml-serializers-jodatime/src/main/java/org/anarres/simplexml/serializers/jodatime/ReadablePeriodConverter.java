...
public class ReadablePeriodConverter extends AbstractConverter implements Converter<ReadablePeriod>, ConverterFactory {
    ...
    public Iterable<? extends Map.Entry<? extends Class<?>, ? extends Converter<?>>> newConverters(Serializer serializer) {
        return Collections.singleton(Maps.immutableEntry(ReadablePeriod.class, this));
    }

    @Override
    public ReadablePeriod read(InputNode node) throws Exception {
        String text = getAttribute(node, ATTRIBUTE);
        return PeriodTransform.parse(text);
    }

    @Override
    public void write(OutputNode node, ReadablePeriod value) throws Exception {
